...
const ReviewForm = ({ setReviews }) => {
  ...
  const post = async (e) => {
    ...
    try {
      ...
      if (file) {
        fileurl = await uploadFile(file, `reviews/${uid}`);
      ...
      const newReview = { ...values, rating, uid, fileurl, created_at };

      const res = await addData("reviews", newReview, uid);
    ...
    <form className={styles.ReviewForm} onSubmit={post}>
      ...
      <CustomInput
        ...
        onChange={(e) => {
          ...
          const acceptedExts = ["jpg", "jpeg", "png", "svg"];
          let fileError = "";
...
const ReviewsScreen = ({ reviews, setReviews, session }) => {
  return (
